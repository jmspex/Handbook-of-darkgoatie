...
module.exports = {
    ...
    if(!message.member.hasPermission('MANAGE_MESSAGES') && !message.member.roles.cache.some((r) => r.name.toLowerCase() === "Giveaway Manager")){
        return message.channel.send(':boom: You need to have the \`MANAGE_MESSAGES\` permissions or a role named Giveaway Manager to end giveaways.');
    }
